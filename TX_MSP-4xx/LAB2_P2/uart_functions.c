...
writeHex (int value)
{
    ...
    int i =0, j=0;
    int temp = 0;
    /* loop till decimal == 0 c*/
    while(value!=0) {
    /*find the 16 modulo of decimal and check whether it is beyond 9, if beyound 9, set 10 =A.. 15=F */
    temp = value % 16;
    if( temp < 10)
    temp =temp + 48; else
    temp = temp + 55;
    hexa[i++]= temp;
    /* assign the remainder of division of 16 to decimal */
    value = value / 16;
    }
    printf("0x");
    for (j = i -1 ;j>= 0;j--)
